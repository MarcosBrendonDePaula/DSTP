...
describe('mod flow_brain.lua — spec normalisation, target decisions, brain swap/restore, commands', () => {
  it('runs the harness', () => {
    const result = runLuaHarness({
      modules: { CORE: modSource('core.lua'), COMMANDS: modSource('commands.lua'), FLOW_BRAIN: modSource('flow_brain.lua'), FLOWBRAIN_COMP: modSource('../components/dstp_flowbrain.lua') },
